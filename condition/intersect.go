...
func NewIntersectCondition(field1, valueType1, field2, valueType2 string, cfg *core.ConditionConfig) *IntersectCondition {
	c := &IntersectCondition{
...
func (c *IntersectCondition) setValueType(valueType1, valueType2 string) error {
...
func (c *IntersectCondition) Expression() string {
	return c.expression
}

func (c *IntersectCondition) BeforeEvaluate(ctx core.EvalContext) error {
	return ctx.BuildEvalParams(c.builtinParams)
}

func (c *IntersectCondition) Evaluate(ctx core.EvalContext) (bool, bool, error) {
	out, _, err := c.program.Eval(ctx.EvalParams())
	return Out(out, err)
}

func (c *IntersectCondition) Negate() (core.Condition, error) {
	n := NewDisjointCondition(c.field1, c.valueTypeStr1, c.field2, c.valueTypeStr2, c.cfg)
...
func (c *IntersectCondition) Compile() error {
...
func (c *IntersectCondition) generateName() string {
	return fmt.Sprintf("intersections of list(%s) and list(%s)", c.field1, c.field2)
}

func (c *IntersectCondition) buildCELProgram(builtinVars map[string]*cel.Type) error {
