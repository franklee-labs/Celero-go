...
func NewNotEqualCondition(field, value string, valueType core.ValueType, cfg *core.ConditionConfig) *NotEqualCondition {
	c := &NotEqualCondition{
...
func (c *NotEqualCondition) Validate() core.Validation {
...
func (c *NotEqualCondition) Expression() string {
	return c.expression
}

func (c *NotEqualCondition) BeforeEvaluate(ctx core.EvalContext) error {
	return ctx.BuildEvalParams(c.builtinParams)
}

func (c *NotEqualCondition) Evaluate(ctx core.EvalContext) (bool, bool, error) {
	out, _, err := c.program.Eval(ctx.EvalParams())
	return Out(out, err)
}

func (c *NotEqualCondition) Negate() (core.Condition, error) {
	n := NewEqualCondition(c.field, c.value, c.valueType, c.cfg)
...
func (c *NotEqualCondition) Compile() error {
	switch c.valueType {
	...
	default:
		return errors.New("Unsupported ValueType in NotEqualCondition.")
	}
}

func (c *NotEqualCondition) generateName() string {
	switch c.valueType {
	case core.ValueTypeString:
...
func (c *NotEqualCondition) buildCELProgram(builtinVars map[string]*cel.Type) error {
